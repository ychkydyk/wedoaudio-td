...
def onFrameStart(frame):
    ...
    try:
        ...
        sr = ext.Samplerate()
        core = ext.core(sr)
        # ПЕРВЫЙ КАДР НЕ ИЗМЕРЯЕТСЯ, И ЭТО ЧЕСТНЕЕ ПОДСТАНОВКИ.
        # Здесь стоял запас на project.cookRate — заявленную частоту кадров.
        # Но заявленная частота это намерение, а не отсчёт: под нагрузкой проект
        # идёт медленнее, чем объявил. Разность двух показаний часов надёжна
        # только на замкнутом промежутке; на первом кадре вычитать нечего, и
        # подстановка выдавала правдоподобное число там, где измерения нет вовсе.
        # Такая ошибка тише отказа: ничего не падает, просто спад огибающих и
        # темп врут во столько раз, во сколько проект не успевает за собственной
        # заявкой. Поэтому третье состояние — не «примерно столько», а «не
        # измерено»: кадр пропускается, и Status говорит об этом прямо.
        now = absTime.seconds
        last = _S.get("last_t", -1.0)
        _S["last_t"] = now
        dt = (now - last) if last >= 0 else -1.0
        if dt <= 0 or dt > 0.5:
            ext.Status = "интервал не измерен (первый кадр или разрыв) — признаки не обновлены"
            return
