...
Файл: `WEDOAUDIO_4.3.0_TD2025.32460.tox`. Собран и проверен непосредственно в
**TouchDesigner 2025.32460**: свежая загрузка под другим именем в чужом контейнере, эталонный
сигнал громкости, правый стереоканал, потеря и возврат входа, смена частоты на 48 кГц, Reset,
намеренные заминки кадра. Итог приёмки лежит в `native_acceptance.json`.
Другую сборку TD для 4.3.0 не проверяли; файлы 4.2.1 в репозитории сохраняют свои версии.

Сборка: `tools/build_portable.py` внутри TD, приёмка: `tools/native_acceptance.py` там же.
Перед запуском задать `WEDOAUDIO_REPO` — папку этой копии репозитория. Сборщик встраивает
текущие исходники из `src/`, создаёт рабочие диапазоны параметров и пишет номер сборки TD в имя файла.

## Установка

Перетащите файл в свою сеть и подайте аудио на вход. Сначала проверьте его в копии проекта.
Компонент содержит только анализатор: 16 внутренних узлов, одна внешняя COMP.
Проверочные источники и пользовательский перформанс в файл не входят.
...
2. **Подайте звук на вход компонента.** Любой аудио-CHOP: `Audio File In`, `Audio Device In`,
   ...
   выставлять не нужно.
3. **Читайте выходы.**
   - `out_features` — CHOP: 22 именованных канала и шесть каналов громкости после них;
   - `out_spectrum` — TOP, спектр для шейдеров.
4. **Проверьте параметр `Status`** на странице `WEDOAUDIO`. Он говорит, что происходит:
   `работает · 48000 Гц · 28 признаков` — всё в порядке; `нет спектра — подключите звук ко входу`
...
a.Loudness()                        # громкость словарём; неизмеренное равно None
a.ResetLoudness()                   # начать интегральную заново — на границе трека
```
...
### Громкость (ещё шесть каналов, тумблер `Loudness`)

| канал | единица | что это |
|---|---|---|
| `lufsm` | LUFS | мгновенная, окно 400 мс |
| `lufss` | LUFS | кратковременная, окно 3 с |
| `lufsi` | LUFS | интегральная с момента сброса, два гейта: −70 LUFS и −10 LU |
| `lra` | LU | диапазон громкости по EBU Tech 3342 |
| `truepeak` | dBTP | истинный пик за последние 400 мс, передискретизация ×8 |
| `truepeakmax` | dBTP | наибольший истинный пик с момента сброса |

Считается по ITU-R BS.1770-4 на **самих отсчётах стереовхода**, до сведения в моно, а не по спектру.
Эти каналы идут строго после базовых 22, поэтому прежние индексы и имена не сдвигаются; с
выключенным `Loudness` шина ровно та же, что была до 4.3.

**`−100` означает «не измерено», а не «очень тихо».** Мгновенной громкости нужны 400 мс звука,
кратковременной — 3 с, диапазону — 4 с, интегральной — хотя бы один блок выше −70 LUFS. В канале
CHOP обязано стоять число, поэтому там `−100`; из Python `Loudness()` отдаёт честный `None`.
В выражениях проверяйте `> -99`, прежде чем пользоваться значением.

`Resetloudness` начинает интегральную и диапазон заново — жмите на границе трека. Разрыв входа и
общий `Reset` делают то же самое. `Truepeak` можно выключить, если пик не нужен: громкость останется.
Цена всего блока — около 0.4 мс на кадр, и она не растёт к концу двухчасового сета.

Громкость кормится каждый кадр, мимо `Rate`: пропущенный кадр здесь не «реже считаем», а выпавший
звук. За этим следит сторож: если за последние 10 с отсчётов пришло заметно меньше, чем прошло
времени по настенным часам, `Status` дописывает `громкость: потеряно N% сэмплов`.
...
- **`bpm` стартует со 120 и стоит там, пока темп не измерен.** Канала уверенности в шине нет,
  поэтому ровный тон без единого удара показывает 120. Это прежнее поведение, и оно расходится
  с нашим же правилом «не измерено вместо правдоподобного числа»; менять значение молча нельзя —
  на него могут делить. Исправление потребует нового канала и пойдёт отдельной версией.
- **Громкость проверена для моно и стерео.** Берутся первые два канала входа; раскладки 5.1 с
  весами тыловых каналов не заявлены. Измеритель **не сертифицирован**: совпадение с эталонными
  сигналами EBU — это проверка, а не аттестат.
- **Интегральная громкость покрывает последние два часа.** Дальше самые старые блоки вытесняются.
- **Кадр длиннее 0.2 с теряет звук, и это свойство TouchDesigner, а не компонента.** Срез времени
  у CHOP ограничен 0.2 с: при заминке 0.4 с во все срезовые операторы приходит 0.2 с звука, а часы
  самого TD сдвигаются на столько же — по ним ничего не пропало. Компонент вернуть этот звук не
  может, но сторож громкости сверяется с настенными часами и сообщает о потере в `Status`.
...
**4.4 — закрыть дрон.** Ложные срабатывания на тянущемся материале сегодня самая заметная
...
**Следом — сэмпл-точные онсеты.** Отдельный путь по времени параллельно кадровому, чтобы
...
(MFCC, хрома), осталось встроить и померить стоимость кадра. Громкость из этого списка
вышла раньше и иначе: в 4.3.0 она считается по отсчётам, а не по спектру. Заодно — канал
уверенности темпа, чтобы `bpm` перестал показывать 120 там, где ничего не измерено.
